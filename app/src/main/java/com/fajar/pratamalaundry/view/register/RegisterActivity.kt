...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun registerAction() {
        ...
        val et_alamat = _binding.etAddress
        ...
        val alamat = _binding.etAddress.text.toString()
        ...
        when {
            ...
            alamat.isEmpty() -> et_alamat.error = "Alamat tidak boleh kosong"
            email.isEmpty() -> _binding.etEmail.error = "Email tidak boleh kosong"
            ...
            password.length < 8 -> et_password.error = "Password kurang dari 8 karakter"
                ...
                val dialodLoadingBuilder = AlertDialog.Builder(this).apply {
                    setView(customBind.root)
                    setCancelable(false)
                }
                val dialogLoading = dialodLoadingBuilder.create()

                registerViewModel.registerUser(nama_pelanggan, no_telp, alamat, email, password)
                    .observe(this) { result ->
                        when (result) {
