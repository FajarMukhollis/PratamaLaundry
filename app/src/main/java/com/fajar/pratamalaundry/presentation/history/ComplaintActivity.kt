...
class ComplaintActivity : AppCompatActivity() {
    ...
    private fun postComplaint(){
        lifecycleScope.launch {
            ...
            val et_complaint = _binding.etComplaint
            val complaint = _binding.etComplaint.text.toString()
            val idTransaksi = intent.getStringExtra("id_transaksi").toString()
            val idPelanggan = intent.getStringExtra("id_pelanggan").toString()
            when{
                complaint.isEmpty() -> et_complaint.error = "Complaint tidak boleh kosong"
                else -> {
                    ...
                    call.enqueue(object : Callback<ComplaintResponse>{
                        override fun onResponse(
                            ...
                            if (response.isSuccessful){
                                ...
                                Toast.makeText(this@ComplaintActivity, message, Toast.LENGTH_SHORT).show()
                                finish()
                            } else {
                                dialogLoading.dismiss()
                                Toast.makeText(this@ComplaintActivity, message, Toast.LENGTH_SHORT).show()
                        ...
                        override fun onFailure(call: Call<ComplaintResponse>, t: Throwable) {
                            dialogLoading.dismiss()
                            Toast.makeText(this@ComplaintActivity, "Gagal Mengirimkan Complaint", Toast.LENGTH_SHORT).show()
                        }
                    })
